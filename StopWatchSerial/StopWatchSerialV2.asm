...
; V 2.0 -- USING AZ80 ASSEMBLER

; TO ASSEMBLE:  ./az80 StopWatchSerialV2.asm -l StopWatchSerialV2.lst
;               -o StopWatchSerialV2.hex (all on one command)
...
DELAYFINE: EQU 07H      ; FINE DELAY TIME
; ADD OR REMOVE NOPS WITHIN DELAY LOOP FOR FINER TUNING
; ADD OR REMOVE NOPS AT END OF FINEDELAY SECTION FOR FINEST TUNING
; (BEFORE RET)
...
ONES:                   ; TIME TO INCREMENT ONES
    ;CALL CLKTONE ; IF YOU WANT THIS, RECALIBRATE THE TIMER.
...
MODE1:	; RUN		    ; IF MODE IS 1 (RUN), WE DON'T NEED TO DO ANYTHING
...
CONTDLY1:
    ...
    ; NOPS FOR TIME CALIBRATION (FINER TUNING)
    NOP
    NOP 
    NOP
    NOP 
    ...
    NOP ;  1/4 SEC EVERY MINUTE FAST -- ADDED FINEDELAY BELOW
...
DELAYRET:
    PUSH BC
    LD B,DELAYFINE ; ADJUST DELAYFINE AT HEAD OF PROGRAM
    ; ALSO SEE NOPS BELOW FOR FINEST TUNING
FINEDELAY:
    NOP
    DJNZ FINEDELAY
    POP BC

    NOP  ; NOPS FOR CALIBRATION -- FINEST DELAY TUNING
    NOP
...
; ** BEGIN CLICKTONE **
CLKTONE:    ; IF YOU WISH TO USE A CLICK TONE EVERY SECOND OR MINUTE,
; CALL THIS ROUTINE AND RE-CALIBRATE THE TIMER.
...
; ** BEGIN FOOTER (DEFINE BYTES (DB)) **
    ORG 2300H		; AVOID CROSSING PAGE BOUNDARY FOR MEMORY BYTES
    
; ** BEGIN DATA TABLES (ARRAYS) **
DELAYTIME: 		; FOR DELAY LOOPS
    DS 2 ; DEFINE SPACE FOR LOW AND HIGH BYTES OF DELAY LOOP
    ; DELETE ;DB 0FFH,0FFH 	; THESE WILL BE OVERWRITTEN WITH
				    ;CONSTANTS DEFINED IN HEADER
    
MESSAGE: ; DIGITS TO DISPLAY
    DS 18 ; DEFINE SPACE FOR MESSAGE
    ; DELETE ;DB 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H, 00H    ; MESSAGE TO DISPLAY

HUNDREDTH:  ; WORKSPACE STARTING AT HUNDREDTHS
    DS 6 ; DEFINE SPACE AS WORKSPACE
    ; DELETE ;DB 00H, 00H, 00H, 00H, 00H, 00H
